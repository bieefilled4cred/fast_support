...
export default function AppSidebar() {
  ...
  const uniqueGroups = Array.from(
    new Set(filteredLinks.map((link) => link.group || "Overview"))
  );

  const groupedLinks = filteredLinks.reduce((acc: any, link: any) => {
    const groupName = link.group || "Overview";
    if (!acc[groupName]) {
      acc[groupName] = [];
    }
  ...
  return (
    <Sidebar variant="sidebar" collapsible="icon">
      ...
      <SidebarContent className="px-[22px] border-t border-b border-[#EAEAEA]">
        {/* authenticated */}
        {false ? (
          ...
          uniqueGroups.map((group) => (
            <SidebarGroup key={group} className="py-[20px] px-0">
              ...
              <SidebarGroupContent>
                <SidebarMenu className="gap-3 mt-[20px]">
                  {groupedLinks[group]?.map((item: any) => (
                    <SidebarMenuItem key={item.title}>
                      <SidebarMenuButton asChild>
                        <Link
                          href={item.url}
                          className={`relative flex items-center  hover: py-[20px] px-[14px] text-[#04B2F1] rounded-none ${
                            (pathname === item.url ||
